...
void RenderMesh::CreateMesh(const VertexFormat *verts, const VERTEX_POS *vertPos, 
							unsigned int numVerts, D3D_PRIMITIVE_TOPOLOGY _primitiveType)
{
	...
	startShadowVertex = 
		VertexBufferManager::GetReference().GetVertexBuffer<VERTEX_POS>().AddVerts(vertPos, numVerts);
...
template <typename VertexFormat>
void RenderMesh::CreateMesh(const VertexFormat *verts, const VERTEX_POSBONEWEIGHT *vertPos, unsigned int numVerts, 
				D3D_PRIMITIVE_TOPOLOGY _primitiveType)
{
	...
	startVertex = VertexBufferManager::GetReference().GetVertexBuffer<VertexFormat>().AddVerts(verts, numVerts);
	startShadowVertex = 
		VertexBufferManager::GetReference().GetVertexBuffer<VERTEX_POSBONEWEIGHT>().AddVerts(vertPos, numVerts);
...
/// \param primitiveType - The type of primitive represented by the vertices.
template <typename VertexFormat>
void RenderMesh::CreateIndexedMesh(const VertexFormat *verts, const VERTEX_POS *vertPos,
								   unsigned int numVerts, const unsigned int *indices, 
								   unsigned int numIndices, D3D_PRIMITIVE_TOPOLOGY primitiveType)
{
	CreateMesh<VertexFormat>(verts, vertPos, numVerts, primitiveType);
...
template <typename VertexFormat>
void RenderMesh::CreateIndexedMesh(const VertexFormat *verts, const VERTEX_POSBONEWEIGHT *vertPos, 
								   unsigned int numVerts, const unsigned int *indices, 
								   unsigned int numIndices, D3D_PRIMITIVE_TOPOLOGY primitiveType)
{
	CreateMesh<VertexFormat>(verts, vertPos, numVerts, primitiveType);
