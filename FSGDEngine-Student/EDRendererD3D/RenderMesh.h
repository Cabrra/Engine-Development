...
namespace EDRendererD3D
{
    ...
    private:
        ...
        UINT                        startVertex;
		// The index location of the first vertex in the vertex buffer for the sahdow mesh.
		UINT                        startShadowVertex;
    ...
    public:
		...
		void CreateMesh(const VertexFormat *verts, const VERTEX_POS *vertPos, unsigned int numVerts, 
			D3D_PRIMITIVE_TOPOLOGY _primitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
		template <typename VertexFormat>
		void CreateMesh(const VertexFormat *verts, const VERTEX_POSBONEWEIGHT *vertPos, unsigned int numVerts, 
			D3D_PRIMITIVE_TOPOLOGY _primitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
		...
		/// \param primitiveType - The type of primitive represented by the vertices.
		template <typename VertexFormat>
        void CreateIndexedMesh(const VertexFormat *verts, const VERTEX_POS *vertPos, 
		...
		template <typename VertexFormat>
        void CreateIndexedMesh(const VertexFormat *verts, const VERTEX_POSBONEWEIGHT *vertPos, 
			unsigned int numVerts, const unsigned int *indices, unsigned int numIndices, 
			D3D_PRIMITIVE_TOPOLOGY primitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
        ...
        inline unsigned int GetShadowStartVertex() const { return startShadowVertex; }
