...
namespace EDRendererD3D
{
	...
	void RenderContext::ContextShadowMappingRenderFunc(RenderNode &node)
	{
		...
		EDRendererD3D::RenderShape *pCurrent = (EDRendererD3D::RenderShape *)(context.GetRenderSet().GetHead());

		Renderer::Render(context.GetRenderSet(), EDRendererD3D::RenderShape::IndexedPrimitiveShadowRenderFunc);
	}

	void RenderContext::ContextShadowMappingSkinnedRenderFunc(RenderNode &node)
	{
		RenderContext &context = (RenderContext &)node;
		// Set the Vertex Layout
		Renderer::theContextPtr->IASetInputLayout(
			InputLayoutManager::GetReference().GetInputLayout(eVERTEX_POSBONEWEIGHT));
		...
		ID3D11Buffer *const vertexBufferPtr = 
			VertexBufferManager::GetReference().GetPosBoneWeightBuffer().GetVertexBuffer();
		Renderer::theContextPtr->IASetVertexBuffers(0, 1, &vertexBufferPtr, &stride, &offset);
		...
		RenderShapeSkinned *pCurrent = 
			(RenderShapeSkinned *)(context.GetRenderSet().GetHead());

		Renderer::Render(context.GetRenderSet(), 
			RenderShapeSkinned::IndexedPrimitiveShadowRenderFunc);
	}
