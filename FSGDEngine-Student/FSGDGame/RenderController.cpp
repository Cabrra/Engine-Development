...
void RenderController::CreateContexts()
{
	...
	shadowMappingContextHandle = ContentManager::LoadXML<RenderContext>(
		"GDRenderContext/rcx_ShadowMapping.xml");
	shadowMappingContextHandle.GetContent()->RenderFunc =
		RenderContext::ContextShadowMappingRenderFunc;

	shadowMappingSkinnedContextHandle = ContentManager::LoadXML<RenderContext>(
		"GDRenderContext/rcx_ShadowMappingSkinned.xml");
	shadowMappingSkinnedContextHandle.GetContent()->RenderFunc =
		RenderContext::ContextShadowMappingSkinnedRenderFunc;
...
void RenderController::PostRender()
{
	...
	gBufferTextureHandler.Activate(gBufferRT, Renderer::GetDepthSRV());

	// Run any post-process effects
	PostProcessController::GetInstance()->Process(sceneTarget);

	gBufferTextureHandler.Revert();
	...
	{
		//if (debugBufferToRender == GB_Diffuse)
		//	debugBufferShape.SetRenderTarget(&gBufferRT, GB_Diffuse - 1);
		//else if (debugBufferToRender == GB_Normal)
		//	debugBufferShape.SetRenderTarget(&gBufferRT, GB_Normal - 1);
		//else if (debugBufferToRender == GB_Specular)
		debugBufferShape.SetRenderTarget(&gBufferRT, debugBufferToRender - 1);
...
void RenderController::RenderShadowCasters(ILight *lightPtr, RenderContext::RenderStage stage)
{
	...
	shadowMappingContextHandle.GetContent()->SetRenderStage(stage);
	shadowMappingContextHandle.GetContent()->RenderProcess();

	// Render skinned animated objects
	shadowMappingSkinnedContextHandle.GetContent()->SetRenderStage(stage);
	shadowMappingSkinnedContextHandle.GetContent()->RenderProcess();
...
void RenderController::RenderPointLight(PointLightSource *lightSourcePtr)
{
	...
	DepthStencilStateManager::GetReference().ApplyState(DepthStencilStateManager::GetReference().DSS_NoDepth);
	BlendStateManager::GetReference().ApplyState(BlendStateManager::GetReference().BS_Additive);
	...
	{
		PointLightWithShadow * lite = (PointLightWithShadow *)lightSourcePtr->GetLightPtr();
		RenderPointDepths(lite);
		sceneTarget.ActivateTarget();
		sceneTarget.ClearDepthStencilView(D3D11_CLEAR_STENCIL);
		lite->ActivateShadowTexture();
		pointLightWithShadowsContextHandle.GetContent()->RenderProcess();
		lite->RevertShadowTexture();
		pointLightWithShadowsContextHandle.GetContent()->ClearRenderSet();
	}
	else
	{
		PointLight  * poLite = (PointLight  *)lightSourcePtr->GetLightPtr();
		poLite->ApplyLight();
		sceneTarget.ActivateTarget();
		sceneTarget.ClearDepthStencilView(D3D11_CLEAR_STENCIL);
...
void RenderController::RenderSpotLight(SpotLightSource *lightSourcePtr)
{
	...
	if (lightSourcePtr->GetShadows() == EDGameCore::ILight::SOFT)
	{
		SpotLightWithShadow* lite = (SpotLightWithShadow *)lightSourcePtr->GetLightPtr();
		RenderSpotDepths(lite);
		sceneTarget.ActivateTarget();
		sceneTarget.ClearDepthStencilView(D3D11_CLEAR_STENCIL);
		lite->ActivateShadowTexture();
	...
	{
		SpotLight  * poLite = (SpotLight  *)lightSourcePtr->GetLightPtr();
		poLite->ApplyLight();
		sceneTarget.ActivateTarget();
		sceneTarget.ClearDepthStencilView(D3D11_CLEAR_STENCIL);
	...
	EDGameCore::Camera *camPtr)
{
	...
	const Aabb &aabb = camPtr[0].GetFrustumAabb();
	...
	//DirectionalLightWithShadow * lite = (DirectionalLightWithShadow *)lightSourcePtr->GetLightPtr();
	//BuildDirViewProjectionCascade(aabb, lite, 0);
	//RenderDirectionalDepths(lite, 0);
	//sceneTarget.ActivateTarget();
	//sceneTarget.ClearDepthStencilView(D3D11_CLEAR_STENCIL);
	//lite->ActivateShadowTexture();
	//GetDirectionalLightWithShadowsContextHandle().GetContent()->RenderProcess();
	//lite->RevertShadowTexture();
	//GetDirectionalLightWithShadowsContextHandle().GetContent()->ClearRenderSet();
	...
	EDRendererD3D::DirectionalLightWithShadow *lightPtr, unsigned int index)
{
	...
	XMFLOAT4X4 aux;
	XMStoreFloat4x4(&aux, XMMatrixOrthographicLH(aabb.Max().x - aabb.Min().x, aabb.Max().y - aabb.Min().y, 0.01f, aabb.Max().z - aabb.Min().z));
	lightProjection = (EDMath::Float4x4&)aux;

	//BuildDirShadowViewProjectionSolution(aabb, lightPtr, index);

	XMFLOAT4X4* copy = (XMFLOAT4X4*)lightPtr->GetWorldMatrixPtr();
	XMFLOAT4 myZ = (XMFLOAT4&)copy->m[2];

	EDMath::Float3 finalZ = (EDMath::Float3&) myZ;
	EDMath::Float3 intermedio = aabb.center - EDMath::Float3(finalZ.x, finalZ.y, finalZ.z) * aabb.extents.r;
	copy->_41 = intermedio.x;
	copy->_42 = intermedio.y;
	copy->_43 = intermedio.z;

	XMMATRIX hereICome;
	XMFLOAT4X4 another;
	hereICome = XMLoadFloat4x4(copy);

	XMMATRIX matrix = XMMatrixInverse(nullptr, hereICome);

	XMStoreFloat4x4(&another, matrix);
	lightView = (EDMath::Float4x4&) another;
	dirViewProjection[index] = (XMFLOAT4X4&)(lightView * lightProjection);

	//BuildDirShadowShimmerFixSolution(index);
	XMFLOAT3 myVec = XMFLOAT3(0, 0, 0);
	XMVECTOR perfect = XMLoadFloat3(&myVec);
	XMMATRIX myMat = XMLoadFloat4x4(&dirViewProjection[index]);
	XMVECTOR point = XMVector3TransformCoord(perfect, myMat);

	XMStoreFloat3(&myVec, point);
	XMFLOAT3 tsPoint;
	tsPoint.x = myVec.x * theDirectionalShadowSize * 0.5f;
	tsPoint.y = myVec.y * theDirectionalShadowSize * 0.5f;
	...
	errorX = errorX / (theDirectionalShadowSize * 0.5f);
	errorY = errorY / (theDirectionalShadowSize * 0.5f);

	XMMATRIX mRound;

	mRound = XMMatrixTranslation(errorX, errorY, 0.0f);

	XMMATRIX seconmd;
	seconmd = XMLoadFloat4x4(&dirViewProjection[index]);
	XMMATRIX sunsent = XMMatrixMultiply(seconmd, mRound);
	XMFLOAT4X4 finale;
	XMStoreFloat4x4(&finale, sunsent);
	dirViewProjection[index] = finale;
}
...
void RenderController::RenderGBuffers()
{
	...
	//for (list<RenderContext *>::iterator helper = gBufferContexts.begin(); helper != gBufferContexts.end(); ++helper)
	//{
	//	(*helper)->SetRenderStage(RenderContext::RS_GBUFFER);
	//	(*helper)->RenderProcess();
	//}

	//TerrainManager::GetReference().RenderTerrain();
...
void RenderController::RenderUnlitObjects()
{
	...
	//gBufferTextureHandler.Revert();
	//BlendStateManager::GetReference().ApplyState(BlendStateManager::GetReference().BS_Default);
	//DepthStencilStateManager::GetReference().ApplyState(DepthStencilStateManager::DSS_LessEqual);

	//for (std::list<RenderContext *>::iterator helper = unlitOpaqueContexts.begin(); helper != unlitOpaqueContexts.end(); ++helper)
	//{
	//	(*helper)->SetRenderStage(RenderContext::RS_UNLIT);
	//	(*helper)->RenderProcess();
	//	(*helper)->ClearRenderSet();
	//}
...
void RenderController::RenderTransparentObjects()
{
	...
	//BlendStateManager::GetReference().ApplyState(BlendStateManager::GetReference().BS_Alpha);
