...
void MouseLookController::OnMessage(EDGameCore::Message& message, const void * data)
{
	...
	else if( message.messageId == MSG_MOUSE_ROTATE )
	{
		...
		POINT difference;
		difference.x = new_point.x - cursorPos.x;
		difference.y = new_point.y - cursorPos.y;
		
		camera_mat.rotateLocalY(difference.x * 0.005f);
		camera_mat.rotateLocalX(difference.y * 0.005f);


		CrossProduct(camera_mat.XAxis, Float3(0.0f, 1.0f, 0.0f), camera_mat.ZAxis);
		camera_mat.XAxis = camera_mat.XAxis.normalize();

		CrossProduct(camera_mat.YAxis, camera_mat.ZAxis, camera_mat.XAxis);
		camera_mat.YAxis = camera_mat.YAxis.normalize();
