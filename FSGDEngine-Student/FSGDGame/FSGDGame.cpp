...
#include "../EDThreadPool/JakeExample.h"
...
void PoolJakeExample(unsigned int _eventId, const void* _eventData, void* _listener)
{
	if (_eventData != nullptr)
	{
		JakeExample* worker = new JakeExample(wcstol((wchar_t*)_eventData, '\0', 10));
		ThreadPool::GetInstance()->PostWork(worker);
		ThreadPool::GetInstance()->testObjects.push_back(worker);
	}
}
...
void Game::Initialize(void) 
{
	...
	InputConsole::GetReference().RegisterCommand(L"JakeExample", 0, PoolJakeExample);
