...
namespace EDGameCore
{
	...
	const Float4x4& Camera::GetViewMatrix(void)
	{
		...
		viewMatrix = this->GetGameObject()->GetTransform()->GetWorldMatrix();
		Float4x4 helper = viewMatrix;
		
		helper.WAxis.x = -DotProduct(viewMatrix.WAxis, viewMatrix.XAxis);
		helper.WAxis.y = -DotProduct(viewMatrix.WAxis, viewMatrix.YAxis);
		helper.WAxis.z = -DotProduct(viewMatrix.WAxis, viewMatrix.ZAxis);
		helper.transpose3x3();
